...
class BithumbCurrencies {
    ...
    func refreshSettingCurrencyKey() {
        self.settingCurrencyKey = self.currencyKey
    }

    func setBithumbCurrenyKey(key: String) {
        if self.currencyKey[key] == 0 {
            self.settingCurrencyCount += 1
            self.currencyKey[key] = 1 // starButton 터치상태 확인하는 데 사용하는 딕셔너리 값
            self.settingCurrencyKey[key] = self.settingCurrencyCount
        } else {
            self.settingCurrencyCount -= 1
            self.currencyKey[key] = 0
    ...
    func setCurrencyData() {
        guard let currencyKeyData = UserDefaults.standard.dictionary(forKey: DataIdentifier.bithumbSettingCurrencyKey) as? [String: Int] else { return }
        self.settingCurrencyKey = currencyKeyData
    }
